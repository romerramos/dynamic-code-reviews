...
UI interaction GIFs are generated by `scripts/qa_gif.rb` from PNG or JPEG browser
frames. The repository includes the pure Ruby image decoders and GIF writer, so
there is no FFmpeg or operating-system image-tool requirement on macOS, Linux or
WSL Ubuntu. The bundled decoders retain their MIT license notices under `vendor/`.
Original browser frames remain available in the report for closer reading.
