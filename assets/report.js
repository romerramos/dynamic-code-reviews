...
(() => {
  ...
  if (review.history) {
    ...
    $('revision-navigation').innerHTML = `<label class="revision-control"><span>Saved review</span><select class="select select-sm select-bordered" aria-label="Review revision">${entries.map(entry => `<option value="${entry.number}" ${entry.number === review.history.revision ? 'selected' : ''}>Revision ${entry.number}${entry.number === review.history.revision ? ' · viewing' : ''}</option>`).join('')}</select></label>`;
    $('revision-navigation').querySelector('select').onchange = event => {
      const number = Number(event.target.value);
      if (entries.some(entry => entry.number === number)) location.href = `${review.history.preview ? 'revisions/' : ''}${String(number).padStart(3, '0')}.html`;
    };
  }
