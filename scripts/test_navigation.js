...
const headings = [100, 400, 700].map((top,i) => ({id:`anchor-${i}`,classList:{contains:name=>name==='code-row',add(){},remove(){}},querySelector:()=>({}),getBoundingClientRect:()=>({top})}));
content.querySelectorAll = selector => selector === '.change-anchor' ? headings : [];
...
const anchors = [250, 1100].map((top,i) => ({id:`anchor-${i}`,classList:{contains:name=>name==='code-row',add(){},remove(){}},querySelector:()=>({}),getBoundingClientRect:()=>({top:top-content.scrollTop})}));
content.querySelectorAll = selector => selector === '.change-anchor' ? anchors : [];
