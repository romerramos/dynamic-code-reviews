...
Order groups by **ease of review**: always start with the group that gives the clearest entry into the change, such as a route, controller action, public service entry point, or the central entity/resource whose contract makes the remaining behavior understandable. Follow the user journey or call flow from that starting point into supporting behavior and persistence; do not default to storage-first or directory order. When no entry point is changed, start with the changed entity/resource or contract that provides the most useful orientation, and explain the unchanged entry point briefly rather than adding unrelated code to the diff.

Apply the same reading order within each group: show the entry point or orienting entity first, then the related implementation and its verification. Keep related models, services, UI and tests together; keep tests next to the behavior they establish when it improves comprehension. A file spanning multiple concerns can appear in multiple layers, with each hunk assigned exactly once. Use as few layers as the change warrants.
